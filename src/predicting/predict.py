...
class Pipe:
    ...
    def png_predict_with_prior(self,im,prior):
        ...
        if ow<300 or oh<300:
            ...
            im = np.asarray(temp)
            temp2 = Image.fromarray(np.squeeze(prior),mode="L")
            temp2 = temp2.resize((512,512))
            prior = np.asarray(temp2)
            prior = np.expand_dims(prior,axis=2)

            print(im.shape,prior.shape)
        ...
        outpng = outpng*250
        if ow!=w or oh!=h:
            outpng = Image.fromarray(outpng.astype(np.uint8),mode="L")
            outpng = outpng.resize((oh,ow))
            outpng = np.asarray(outpng)
        return outpng
...
#                preds = (preds>150)
    ...
    def eval_dir_AP(self,GT):
        ...
        with open(os.path.join(self.out_raw+'/Prec_recall.txt'),'w') as FILE:
            ...
            for  name in imlist:
                preds = misc.imread(os.path.join(self.out_raw,name))
                labs = misc.imread(os.path.join(GT,name))
                labs = (labs>0.5).astype(np.float)
                
                preds = preds.astype(np.uint8)
                #AP = ap_score(labs.flatten(),preds.flatten())
                preds = (preds>=(threshold)).astype(np.float)
    ...
    def RStestset_prior(self):
        self.output = opt.output +'/'  +opt.testset+ '_' + self.m_name+'_' + str(self.opt.which_epoch) + '/' 
