...
class PriorWeaklyAnnoDataset(BaseDataset):
    ...
    def __getitem__(self,index):
        ...
        '''
        if random.random()<self.opt.s_strong:
            choosen_set='strong_only'
        else:
            choosen_set='weak_only'
        '''
...
#            translating_= np.random.uniform(0,0.1,1)
#            rotating_ = np.random.randint(0,1,1)[0]
#            affine = T.Compose([
...
#
#            clor = T.Compose([
#                    T.ColorJitter(brightness=0.5, contrast=0.3, saturation=0.3, hue=0.0)])
#            A_img = clor(A_img)
