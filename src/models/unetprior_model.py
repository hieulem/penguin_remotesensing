...
class UnetPriorModel(BaseModel):
    def name(self):
        return 'UnetModelWithPriorBranch'
    def eval(self):
        self.netG.eval()
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, 1, 64,
                                      'unet_prior', opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids,opt=opt)
...
#        print('-----------------------------------------------')
    ...
    def forward(self):
        ...
        self.output = self.netG(self.input,self.prior)
    ...
    def get_prediction_tensor(self,input_A,input_B):
        if len(self.gpu_ids) > 0:
            ...
            input_B = input_B.cuda(self.gpu_ids[0], async=True)
        self.input = Variable(input_A)
        self.output = self.netG(self.input,input_B)
