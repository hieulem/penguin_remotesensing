...
class Unet4cModel(BaseModel):
    def name(self):
        return 'UnetModel with 4 channel'
    def eval(self):
        self.netG.eval()
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc+1, 1, 64,
                                      'unet_128', opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids,opt=opt)
...
#        print('-----------------------------------------------')
    ...
    def set_input(self, input):
        ...
        self.prior = Variable(input['prior'].cuda(self.gpu_ids[0], async=True))
       # self.GT[self.GT<0] = 0
    ...
    def backward_G(self):
        ...
        #regression loss:
        self.loss_G_regression = self.criterion(self.output.mean(dim=0),self.GT.mean(dim=0)) * self.opt.lambda_regression
        #blurry_prediction =  self.output[(self.output>0) & (self.output<1)]
        #self.loss_G_entropy = -blurry_prediction
        self.loss_G = self.loss_G_segmentation + self.loss_G_regression 
        self.loss_G.backward()

    def get_current_visuals(self):
       	self.visual_names = ['input','output','GT','prior'] 
        ...
        for i in range(0,min(nim-1,5)):
            ...
            if self.isweak[i] == 0:
                ...
                row = ImageOps.expand(row,border=4,fill=(0,200,0))
                row = np.asarray(row)
        ...
        all = tuple(all)
        allim = np.vstack(all)
        Image.fromarray(allim).save('test.png')
    ...
    def get_prediction_tensor(self,input_A,input_B):
        if len(self.gpu_ids) > 0:
            input_A = input_A.cuda(self.gpu_ids[0], async=True)
            input_B = input_B.cuda(self.gpu_ids[0], async=True)
        self.prior = Variable(input_B.cuda(self.gpu_ids[0],async=True))
        self.input = Variable(input_A)
        input_ = torch.cat((self.input,self.prior),1)
        self.output = self.netG(input_)
    ...
    def get_prediction(self,input):
        ...
        self.prior = Variable(input['P'].cuda(self.gpu_ids[0],async=True))
        input_ = torch.cat((self.input,self.prior),1)
        self.output = self.netG(input_)
        raw = self.output.data.cpu().float().numpy()
